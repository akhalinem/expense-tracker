...
export function displayDate(date: string) {
  return new Date(date).toLocaleString(process.env.EXPO_PUBLIC_LOCALE, {
    dateStyle: 'full',
  });
}

export const mapCategoryToCategoryExcelDto = (
  category: Category
): CategoryExcelDto => ({
  name: category.name,
...
export const mapTransactionToIncomeExcelDto = (
...
export const mapTransactionToExpenseExcelDto = (
  transaction: Transaction
): ExpenseExcelDto => ({
  amount: transaction.amount,
  description: transaction.description,
