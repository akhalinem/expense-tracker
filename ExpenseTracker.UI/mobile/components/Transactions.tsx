  ...
  return (
    ...
    >
      ...
      >
        <View
          style={[
            styles.transactionItemContent,
            { backgroundColor: theme.background },
          ]}
        >
          <View style={styles.iconContainer}>
            <View
              style={[styles.icon, { backgroundColor: icon.backgroundColor }]}
            >
...
const DateHeader: FC<{ date: string; dailyExpense: number }> = ({
  date,
  dailyExpense,
}) => {
  const { theme } = useTheme();
...
const ListItemRenderer = (props: ListRenderItemInfo<ListItem>) => {
  ...
  if (item.type === ItemType.HEADER) {
    return <DateHeader date={item.date} dailyExpense={item.dailyExpense} />;
  } else {
...
const getListData = (transactions: Transaction[]) => {
  ...
  const dailyExpenses = Object.entries(groupedTransactions).reduce(
    (stats, [date, items]) => {
