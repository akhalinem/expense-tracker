    ...
    <ThemedView style={styles.container}>
      ...
      <KeyboardDismissing>
        {type === TransactionTypeEnum.EXPENSE && (
          <ExpenseForm data={transaction} onClose={onClose} />
        )}
        {type === TransactionTypeEnum.INCOME && (
          <IncomeForm data={transaction} onClose={onClose} />
        )}
      </KeyboardDismissing>
    </ThemedView>
  );
...
const Header: FC<{ type: TransactionTypeEnum; isEditing?: boolean }> = ({
  type,
  isEditing,
}) => {
  return (
...
const SegmentedControlHeader: FC<{ type: TransactionTypeEnum }> = ({
  type,
}) => {
  const { theme } = useTheme();
