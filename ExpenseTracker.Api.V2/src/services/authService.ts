...
import { IAuthResponse, ISession, IValidationResult } from "../types";
...
class AuthService {
  ...
  async validateResetSession(
    ...
    try {
      ...
      if (type !== "recovery") {
        throw new Error(
          "Invalid session type for password reset. Only recovery sessions are allowed."
        );
      }
    ...
    try {
      ...
      if (updateError) {
        console.error("Password update error details:", {
          ...
          statusCode: (updateError as any).status_code,
      ...
      if (this.isAuthError(error)) {
        console.error(
          "AuthError being re-thrown:",
          (error as IAuthError).message
        );
        throw error;
      }
   ...
   */
  createAuthError(error: unknown): IAuthError {
    const authError = new Error() as IAuthError;
    authError.name = "AuthError";

    // Type guard for error with expected properties
    ...
    authError.statusCode = typedError.status || 400;
    ...
    } else if (typedError.message?.includes("Invalid login credentials")) {
      authError.message =
    ...
    } else if (typedError.message?.includes("Too many requests")) {
      authError.message =
        "Too many attempts. Please try again in a few minutes.";
      authError.code = "RATE_LIMITED";
    } else if (typedError.message?.includes("User not found")) {
      authError.message =
        "No account found with this email. Please check your email or create a new account.";
      authError.code = "USER_NOT_FOUND";
    } else {
      authError.message = typedError.message || "Authentication failed";
      authError.code = typedError.code || "AUTH_ERROR";
    }
   ...
   */
  getPasswordUpdateErrorMessage(error: unknown): string {
    const typedError = error as { message?: string };

    if (typedError.message?.includes("New password should be different")) {
   ...
   */
  formatAuthResponse(data: unknown, message: string): IAuthResponse {
    const typedData = data as {
      user: { id: string; email: string; confirmation_sent_at?: string };
      session?: {
        access_token: string;
    ...
    if (typedData.session) {
      response.session = {
        access_token: typedData.session.access_token,
        refresh_token: typedData.session.refresh_token,
        expires_at: typedData.session.expires_at,
        token_type: typedData.session.token_type,
      };
    } else if (typedData.user?.confirmation_sent_at) {
      // Email confirmation required
      response.message =
        "Account created successfully! Please check your email to confirm your account.";
      (response as { confirmation_sent_at?: string }).confirmation_sent_at =
        typedData.user.confirmation_sent_at;
    }
   ...
   */
  private isAuthError(error: unknown): error is IAuthError {
    return (
      typeof error === "object" &&
      error !== null &&
      "name" in error &&
